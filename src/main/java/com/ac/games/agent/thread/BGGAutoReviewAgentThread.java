...
public class BGGAutoReviewAgentThread extends Thread {
  ...
  private void executeMainTask() {
    ...
    System.out.println ("Running the following Query: db.bgggame.find( { reviewState: 0, minPlayers: {$exists:false}} )");
    System.out.println ("The actual object: " + queryObject.toString());
    ...
    System.out.println ("Running the following Query:\n  db.bgggame.find( { reviewState: 0, gameType: {$in: [0, 2]}, publishers : {$exists:true}, publishers.1: {$exists:false}} )");
    System.out.println ("The actual object: " + queryObject.toString());
    //value = console.readLine("< Press Enter to Proceed >");
    //if (value.equalsIgnoreCase("q"))  {
    //  try { client.close(); } catch (Throwable t) { /** Ignore Errors */ }
