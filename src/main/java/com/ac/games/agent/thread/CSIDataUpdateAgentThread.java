...
public class CSIDataUpdateAgentThread extends BackgroundAgentThread {
  ...
  private void executeSimpleTask() {
    ...
    //Step 1 - Get the list of csiIDs from database
    List<Long> gamesToSearch = null;
    try {
      gamesToSearch = database.getCSIIDList();
    ...
    System.out.println ("The number of games we are going to update is: " + gamesToSearch.size());
    ...
    if (gamesToSearch.size() > 0) {
      ...
      for (int pos = 0; pos < gamesToSearch.size(); pos++) {
        ...
        if (game != null) {
          ...
          try {
            database.updateCSIPriceData(game);
          } catch (DatabaseOperationException doe) {
            doe.printStackTrace();
          } catch (ConfigurationException ce) {
            ce.printStackTrace();
