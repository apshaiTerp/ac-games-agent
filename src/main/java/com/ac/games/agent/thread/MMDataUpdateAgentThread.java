...
public class MMDataUpdateAgentThread extends BackgroundAgentThread {
  ...
  private void executeSimpleTask() {
    ...
    //Step 1 - Get the list of mmIDs from database
    List<Long> gamesToSearch = null;
    try {
      gamesToSearch = database.getMMIDList();
    ...
    if (gamesToSearch.size() > 0) {
      ...
      for (int pos = 0; pos < gamesToSearch.size(); pos++) {
        ...
        if (game != null) {
          ...
          try {
            database.updateMMPriceData(game);
          } catch (DatabaseOperationException doe) {
            doe.printStackTrace();
          } catch (ConfigurationException ce) {
            ce.printStackTrace();
